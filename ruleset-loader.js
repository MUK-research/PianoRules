...
export function resolveRulesetCandidates(rawRef, appUrl=(typeof location!=='undefined'?location.href:'https://example.invalid/')) {
  ...
  // Short library names are a deliberate convention: Library/<Name>/<Name>.rules
  ...
  // Relative/absolute direct .rules paths are accepted as-is.
  if(!looksLikeUrl(ref)){
  ...
  if(host==='github.com'||host==='www.github.com'){
    ...
    // Exact GitHub file link: /owner/repo/blob/<branch>/<path>
    ...
    // Repository convention: Rule-Gamelan -> Gamelan.rules, Gamelan -> Gamelan.rules.
    // If a tree URL supplies a branch, prefer that branch; otherwise try main, then master.
    const branch=parts[2]==='tree'&&parts[3]?parts[3]:null;
    const piece=repoPieceName(repo);
    const filenames=[`${piece}.rules`];
  ...
  // Any other public host: use an exact .rules URL. This keeps the convention deterministic.
...
export async function loadRuleset(rawRef,{appUrl=(typeof location!=='undefined'?location.href:'https://example.invalid/'),fetchImpl=(typeof fetch!=='undefined'?fetch:null)}={}){
  ...
  const resolved=resolveRulesetCandidates(rawRef,appUrl);
  const attempts=[];
  for(const url of resolved.candidates){
    try{
      ...
      if(!source.trim()){attempts.push(`empty file ${url}`);continue;}
  ...
  throw new Error(`Could not load ruleset “${resolved.label}”.${detail}`);
}
...
export function resolveRulesetAssetUrl(source,{assetUrls=new Map(),assetBaseUrl=''}={},rulesBaseUrl='',appUrl=(typeof location!=='undefined'?location.href:'https://example.invalid/')){
  const originBase=rulesBaseUrl||appUrl;
  const explicit=assetUrls?.get?.(source);
  ...
  if(assetBaseUrl){
    const base=new URL(assetBaseUrl,originBase),baseHref=base.href.endsWith('/')?base.href:base.href+'/';
...
export function rulesetFromLocation(loc=(typeof location!=='undefined'?location:null)){
  if(!loc)return'';
  return new URLSearchParams(loc.search||'').get('ruleset')||'';
}
